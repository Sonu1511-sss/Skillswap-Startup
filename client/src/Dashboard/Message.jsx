  ...
  return (
    <div className="flex h-[calc(100vh_-_8rem)] bg-white pt-12 rounded-lg overflow-hidden border">
      ...
      <div className="flex-1 flex flex-col bg-gray-50 relative">
        <button onClick={() => setSidebarOpen(!sidebarOpen)} className="md:hidden p-2 bg-blue-500 text-white rounded-full shadow absolute top-3 left-3 z-30"><Menu size={20} /></button>
        {activeChat ? (
          <>
              ...
              {messages.map((m) => {
                  ...
                  return (
                      <div key={m._id || new Date(m.createdAt).toISOString()} className={`flex ${isSender ? "justify-end" : "justify-start"}`}>
                          <div className={`max-w-md lg:max-w-lg px-4 py-2 rounded-2xl shadow break-words ${isSender ? "bg-blue-500 text-white rounded-br-none" : "bg-white text-gray-800 rounded-bl-none border"}`}>
            ...
            <div className="p-4 border-t bg-white flex items-center gap-2">
              ...
              <button onClick={sendMessage} className="bg-blue-500 text-white p-3 rounded-full shadow hover:bg-blue-600 transition"><Send size={18} /></button>
            </div>
          </>
        ) : (
